...
#define COLDET_V2_5 // COLDET_V1 // COLDET_V2 // COLDET_V2_5 // COLDET_V3
...
public:
    ...
    Mat K;
    Mat R;
    // The following matrices are for calibration of the camera with Calibration Matrix and Distortion Matrix, which are the output of the openCV chess-pattern calibration
            ...
            if (srcFree.available()) {
                ...
                if (type == IMAGES) {
                    ...
                    if (!cap.isOpened()) {
                        ...
                        // Camera on Table - old
                        //                        camCalibMat.at<double>(0,0) = 7.2334682506533118e+03;
                        //                        camCalibMat.at<double>(0,2) = 9.4914536747589386e+02;
                        //                        camCalibMat.at<double>(1,1) = 7.5296548611846756e+03;
                        //                        camCalibMat.at<double>(1,2) = 5.6559482327247304e+02;

                        //                        camDistMat.at<double>(0,0) = -4.3907015218905441e+00;
                        //                        camDistMat.at<double>(1,0) = 7.3481585281996985e+01;
                        //                        camDistMat.at<double>(2,0) = 1.2371157416487333e-01;
                        //                        camDistMat.at<double>(3,0) = 4.2359100343860014e-02;

                        // Camera on Table
                        //                        camCalibMat.at<double>(0,0) = 7.4327376594765083e+03;
                        //                        camCalibMat.at<double>(0,2) = 9.6740474590736562e+02;
                        //                        camCalibMat.at<double>(1,1) = 7.7861187621150930e+03;
                        //                        camCalibMat.at<double>(1,2) = 5.7353459368997653e+02;

                        //                        camDistMat.at<double>(0,0) = -4.1464141563746075e+00;
                        //                        camDistMat.at<double>(1,0) = 6.1789012691552379e+01;
                        //                        camDistMat.at<double>(2,0) = 9.2405501875720580e-02;
                        //                        camDistMat.at<double>(3,0) = 4.6869946919607372e-02;


                        // camera on Floor FEB 24
                        //                        camCalibMat.at<double>(0,0) = 9.4830703101416511e+03;
                        //                        camCalibMat.at<double>(0,2) = 9.4822515959479267e+02;
                        //                        camCalibMat.at<double>(1,1) = 9.6898293136381635e+03;
                        //                        camCalibMat.at<double>(1,2) = 6.0099748193336984e+02;

                        //                        camDistMat.at<double>(0,0) = -6.4769041949303441e+00;
                        //                        camDistMat.at<double>(1,0) = 1.8853784942165373e+02;
                        //                        camDistMat.at<double>(2,0) = 1.0341935176689362e-01;
                        //                        camDistMat.at<double>(3,0) = 4.4108030931552321e-02;


                        // camera on Floor MAR 24
                        camCalibMat.at<double>(0,0) = 5.1659949676638944e+03;
                        camCalibMat.at<double>(0,2) = 9.6274691319671945e+02;
                        camCalibMat.at<double>(1,1) = 5.3192876961861375e+03;
                        camCalibMat.at<double>(1,2) = 6.1241801534521937e+02;

                        camDistMat.at<double>(0,0) = -2.0362553322922619e+00;
                        camDistMat.at<double>(1,0) = 1.6092341722302553e+01;
                        camDistMat.at<double>(2,0) = 4.6277861563787312e-02;
                        camDistMat.at<double>(3,0) = -2.8127118089342268e-04;
                    ...
                    if (cap.isOpened()) {
...
#if USE_PYLON       // MOHSEN: Undistort Image
                        ...
                        if(this->bool_halfSize)
                        {
                            // MOHSEN NEW TRY for resizing the img
...
void KilobotTracker::LOOPiterate()
{
    ...
    if ((srcUsed.available() > 0) || this->loadFirstIm)
    {
        ...
        cuda::cvtColor(temp,this->grayImage, cv::COLOR_BGR2GRAY); // or RGB2GRAY?
        ...
        this->fullImagesHSV[0] = channelsHSV[0];
        this->fullImagesHSV[1] = channelsHSV[1];
        this->fullImagesHSV[2] = channelsHSV[2];
        ...
        //        cv::Mat tempB;
        //        resultB.download(tempB);
        //        imshow("result B: ", tempB);
        ...
        // MOHSEN: on making HSV channels!
        cv::cuda::GpuMat resultH(dummyVar2000, dummyVar2000, this->fullImagesHSV[0].type());
        //cv::cuda::resize(this->fullImages[0], this->fullImages[0], cv::Size(2000, 2000), 0,0, cv::INTER_CUBIC);
        this->fullImagesHSV[0].copyTo(resultH);

        cv::cuda::GpuMat resultS(dummyVar2000, dummyVar2000, this->fullImagesHSV[1].type());
        //cv::cuda::resize(this->fullImages[1], this->fullImages[1], cv::Size(2000, 2000), 0,0, cv::INTER_CUBIC);
        this->fullImagesHSV[1].copyTo(resultS);

        cv::cuda::GpuMat resultV(dummyVar2000, dummyVar2000, this->fullImagesHSV[2].type());
        //cv::cuda::resize(this->fullImages[2], this->fullImages[2], cv::Size(2000, 2000), 0,0, cv::INTER_CUBIC);
        this->fullImagesHSV[2].copyTo(resultV);
#else
        ...
        if (flipangle!=0) {
            ...
            // MOHSEN: on HSVing
            this->finalImageH = resultH;
            this->finalImageS = resultS;
            this->finalImageV = resultV;
        }


        //        qDebug() << "Mohsen wrote -- exp Type: " << this->expType;
        if(true)//!loadFirstIm)
            //            qDebug() << "experiment type is: " << this->expType <<", User experiment is: " << USER_EXP;
...
void KilobotTracker::SETUPfindKilobots()
{
    ...
    if ((srcUsed.available() > 0) || this->loadFirstIm)
    {
        ...
        temp.download(saveIm);
        CV_NS split(temp, channels);
        cuda::cvtColor(temp,this->grayImage, cv::COLOR_RGB2GRAY);
        this->fullImages[0] = channels[0];
        this->fullImages[1] = channels[1];
        this->fullImages[2] = channels[2];
    ...
    if (this->grayImage.empty()) return;
...
void KilobotTracker::trackKilobots()
{
    ...
    // MOHSEN: CHECK the above lines later! are they necessary?
    display = this->finalImageCol;
...
//                            }
            ...
            for (int i = 0; i < this->kilos.size(); ++i) {
                ...
                if (this->showIDs) {
                    putText(display, QString::number(this->kilos[i]->getID()).toStdString(), Point(kilos[i]->getPosition().x(),kilos[i]->getPosition().y()-50), FONT_HERSHEY_PLAIN, 2.5, rgbColor, 3);
                }
            }
            this->drawOverlay(display);
    ...
    case CIRCLES_LOCAL:
            ...
            cv::imshow("Local Circles: ", display);
    ...
    case MY_HAPPY_OTHER_TRACKER:
            ...
            // setup the tracking region around each KB's last known position
            float extensionScale = 1.0;
            float maxDist = extensionScale* (float)this->kbMaxSize;
            ...
            for (uint i = 0; i < (uint) this->kilos.size(); ++i) {
                //                Rect bb = bbs[i];
                Rect bb = Rect2d(bbs[i].x - maxDist, bbs[i].y + maxDist, (int) 2*maxDist,(int) 2*maxDist);
                //                bb.
#ifdef USE_CUDA
                cuda::GpuMat temp[3];
#else
                ...
                if (this->t_type & POS) {
                    ...
                    while (no_match && circle_acc > 15) {
#ifdef USE_CUDA
                        cuda::GpuMat circlesGpu;
                        this->hough->setVotesThreshold(circle_acc);
                        //                        this->hough->detect(temp[0],circlesGpu,stream);
                        this->hough->detect(this->grayImage,circlesGpu,stream); // Mohsen: still don't get why should we use BLUE CHANNEL?!!
                    ...
                    //                    imshow("circles Cpu: ", circlesCpu);

                    //                    qDebug() << " .... hey hey : circ size: " << circles.size() << ", circl Acc: " << circle_acc;

                    if (circles.size() > 0) {
                        // take the nearest one
                        float reduceMaxSpeed = 0.0;
                        int best_index = 0;
                        for (uint k = 0; k < circles.size(); ++k) {
                            bool oops = false;
                            // full exclusion
                        ...
                        //                        qDebug() << " .... hey hey : circ size: " << circles.size() << ", circl Acc: " << circle_acc << ", Best Index: " << best_index;

                        if (circles.size() != 0) {


                            // if we haven't moved too far
                            if (true) //(this->kbMaxSize-reduceMaxSpeed)*(this->kbMaxSize-reduceMaxSpeed) > (circles[best_index][0]-bb.width/2)*(circles[best_index][0]-bb.width/2)+(circles[best_index][1]-bb.height/2)*(circles[best_index][1]-bb.height/2))
                            {

                                float smooth_fact = 1.0;//0.99;
                                ...
                                //                                qDebug() << "new Pos: " << new_x << ", " << new_y;

                                newPos = QPointF(new_x,new_y);
                                newVel = prevVel*(2.0f/3.0f) + 0*(newPos - prevPos)*(1.0f/3.0f);
    ...
    case MORFI_TRACKER:
            ...
            //            qDebug() << "Loop Iterate: I reached here: <<<< ";
            //***************************************************************
            // FOR DEGUB: draw all the circles found through GPU-hough
            //            vector<Vec3f> circles;
            //            circlesGpu.download(circles);
            ...
            //            imshow("traacking result: ", display);
            //#endif
            ...
            //            qDebug() << "circChances size: " << circChans[0].size().width;
            if (circChans[0].size().width  > 0) {
                // expand circle x's & y's
                vector < cuda::Stream > streams(4);
                cuda::gemm(circChans[0], ones_kb, 1.0, noArray(), 0.0, all_x_c,GEMM_1_T,streams[0]);
                ...
                for (int i = 0; i < this->kilos.size(); ++i)
                {
                    // find the closest circle (this operation is much quicker on the CPU than on GPU!) // MOHSEN : << check this performance (esp. with large pop.)! REF: XXTRACK_01
                    minMaxLoc(localDists(Rect(i,0,1,localDists.size().height)),min,NULL,minLoc,NULL);
                    //                    cv::cuda::minMaxLoc(all_x_c(Rect(i,0,1,localDists.size().height)),min,NULL,minLoc,NULL);
                    if (this->lost_count[i]>0) previously_lost[i] = this->lost_count[i];
                    // work out if we should update...
                    //                    float var1 = 2;1.15; // MOHSEN: COMMENT REF: XXTRACK_01
                    int var2 = 30; // MOHSEN: Check these one/two variables!! ORG: 3?
                    //                    if (*min < float(this->kbMinSize)/var1 && this->lost_count[i] < var2) { // check if the distance between the old and new position is small enough // MOHSEN REPLACE THIS with the following line
                    //                    qDebug() << "Morfi Displacement Var:" << this->morfiTrack_maxDisplacement;
                    ...
                    if (*min < this->morfiTrack_maxDisplacement && this->lost_count[i] < var2) { // check if the distance between the old and new position is small enough
                        ...
                        // and on the cpu
                        //                        kilos[i]->updateState(QPointF(circChansXCpu.at<float>((*minLoc).y),circChansYCpu.at<float>((*minLoc).y)),kilos[i]->getVelocity(), kilos[i]->getLedColour());
                        QPointF curPnt = QPointF(circChansXCpu.at<float>((*minLoc).y),circChansYCpu.at<float>((*minLoc).y));
                        int new_x = float(curPnt.x())*(1.0-smooth_fact) + float(kilos[i]->getPosition().x())*smooth_fact;
                        int new_y = float(curPnt.y())*(1.0-smooth_fact) + float(kilos[i]->getPosition().y())*smooth_fact;
                        kilos[i]->updateState(QPointF(new_x,new_y),kilos[i]->getVelocity(),kilos[i]->getLedColour());
                    }
                    else {
                        // we could not assign a circle onto the kilobot! maybe we lost the tag?
                        this->lost_count[i]++;
                        ...
                        /*** if the area to search is small enough it might quicker to crop the image, find again cirlces and loop over a smaller set ***/
                        int lostCountVarMin = 20, lostCountVarMax = 150; // MOHSEN: ORG(10,150); Play with these two variables to get the perfomance when losing a kilobot.
                        // For example, if you reduce the EFWL value, then the search area for the lost kilobot will shrink
                        if (this->lost_count[i] > lostCountVarMin && this->lost_count[i] <= lostCountVarMax) {

                            //                            float EFWL = 5.0; // Expansion Factor for search When Lost
                            //                            Rect bb = this->getKiloBotBoundingBox(i, this->lost_count[i]*EFWL/this->kbMaxSize);
                            Rect bb = this->getKiloBotBoundingBox(i, 0.01*this->lost_count[i]*EFWL*this->kbMaxSize);
#ifdef USE_CUDA
                            cuda::GpuMat temp;
#else
                            Mat temp[3];
                            ...
                            for (uint c = 0; c < circles_rematch.size(); ++c) {
                                ...
                                if (!foundOverlapWithOtherKilobot) {
                                    if(debugBool)
                                        qDebug() << "Lost bot #" << kID << "has been Found through local search";
                                    // pair up on CPU!
                                    int new_x = float(cur_x)*(1.0-smooth_fact) + float(kilos[i]->getPosition().x())*smooth_fact;
                                    int new_y = float(cur_y)*(1.0-smooth_fact) + float(kilos[i]->getPosition().y())*smooth_fact;
                                    kilos[i]->updateState(QPointF(new_x,new_y),kilos[i]->getVelocity(),kilos[i]->getLedColour());
                                    //                                    kilos[i]->updateState(QPointF(cur_x,cur_y),kilos[i]->getVelocity(),kilos[i]->getLedColour());
                                    // and GPU
                                    kbLocsCpuX(Rect(i,0,1,1)) = cur_x;
                        ...
                        else if (this->lost_count[i] > lostCountVarMin)  /* go through circles on full image */
                        {
                            // go through xCpu and yCpu (circle locs) and compare to KB locations
                            // Point(mean(xCpu(Rect(i,0,1,1)))[0],mean(yCpu(Rect(i,0,1,1)))[0])
                            for (int c = 0; c < xCpu.size().width; ++c)
            ...
            if (this->t_type & ROT){
                //                float smooth_fact = 0.5;
                //                qDebug() << "smoothness factor is: " << smooth_fact;
                for (int i = 0; i < this->kilos.size(); ++i) {
                    int new_x = float(kilos[i]->getPosition().x())*(1.0-smooth_fact) + float(previousPositions[i].x())*smooth_fact;
                    int new_y = float(kilos[i]->getPosition().y())*(1.0-smooth_fact) + float(previousPositions[i].y())*smooth_fact;
                    ...
                    //                    QPointF newVel = prevVel*(2.0f/3.0f) + (newSmoothPos - previousPositions[i])*(1.0f/3.0f);
                    QPointF newVel = prevVel*smooth_fact + (newSmoothPos - previousPositions[i])*(1.0-smooth_fact);
...
// Color detection V2.5: Mohsen's modified version from 2
#ifdef COLDET_V2_5
void KilobotTracker::getKiloBotLights(Mat &display) {
    // use CUDA to find the kilobot lights...
    ...
    cuda::multiply(colorimagechans[0],redHThreshold,channelR_G,1,-1,stream1);
    cuda::multiply(colorimagechans[2],blueHThreshold,channelB_G,1,-1,stream3);
