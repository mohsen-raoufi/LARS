...
bg_color = (0.5, 0.5, 0.5)  # gray color in RGB format
...
def generate_grid_image(grid_size = (3,3), circle_radius = 1, point_distance = 2, image_size = (100, 100), output_path= "grid_image.png", circle_perfection = 1.0):
    ...
    empty_image[:] = np.array(bg_color) * 255  # fill with the background color
    ax.imshow(empty_image, extent=(0, image_size[0], 0, image_size[1]))

    pos_array = []
    for i in range(grid_size[0]):
        for j in range(grid_size[1]):
            x = x_offset + i * point_distance - circle_radius / 2
            y = y_offset + j * point_distance - circle_radius / 2
    ...
    #     log_file = log_pos_array(pos_array, log_file)
    #     log_file.close()
...
def generate_robot_grid_image(robot_image_path, grid_size=(3, 3), robot_width=50, point_distance=100, image_size=(2000, 2000), output_path="robot_grid_image.png", log_output_path=None):
    """
    ...
    for i in range(grid_size[0]):
        for j in range(grid_size[1]):
            ...
            # rotate the image of the robot based on the theta
            ax = draw_robot_at_pos_w_rot(robot_image, theta=0, x=x, y=y, ax=ax)
...
def generate_robot_grid_from_number(robot_image_path, num_robots=4, robot_width=50, image_size=(2000, 2000), output_path="robot_grid_image.png", log_output_path=None):
    """
    Generates a grid of robot images based on the total number of robots.

    Args:
        robot_image_path (str): Path to the robot image file.
        number_of_robots (int): The total number of robots to display.
        robot_width (int): Desired width for each robot image in pixels.
        image_size (tuple): Size of the output image.
        output_path (str): Path to save the output image.
        log_output_path (str): Path to save the log file.
    """
    ...
    point_distance_x = int((image_size[0] - grid_width*robot_width)/grid_width)
    point_distance_y = int((image_size[1] - grid_height*robot_width)/grid_height)

    # Calculate the offset to center the grid in the image
    # x_offset = (0*image_size[0] + robot_width) / 2
    # y_offset = (0*image_size[1] + robot_width) / 2
    ...
    for i in range(grid_size[0]):
        for j in range(grid_size[1]):
            x = x_offset + (i * (point_distance_x+robot_width)) # - robot_width / 2
            y = y_offset + (j * (point_distance_y+robot_width)) # - robot_width / 2
            # ax.imshow(robot_image, extent=(x, x + robot_image.shape[1], y, y + robot_image.shape[0]))

            # print(f"Drawing robot {i},2000 {j} at position: ({x}, {y})")
            # Draw the robot at the current position
            # rotate the image of the robot based on the theta
            ax = draw_robot_at_pos_w_rot(robot_image, theta=0, x=x, y=y, ax=ax)
...
def generate_robot_image_with_random_robots(robot_image_path, num_robots=10, robot_width=30, image_size=(2000, 2000), 
                                            output_path="robot_image_with_random_robots.png", log_output_path=None):
    ...
    for x, y in robots:
        ...
        ax = draw_robot_at_pos_w_rot(robot_image, theta, x, y, ax)
    ...
    for i_node in range(len(trajectory) - 1):
        ...
        for step in range(steps + 1):
            ...
            if log_output_path:
                ...
                # add the frame number at the begining of the line
                log_file.write(f"{frame_number:03d} ")
                log_file = log_pos_array(pos_array, log_file)
...
def generate_robot_animation_random_robots(robot_image_path, num_robots=10, robot_width=30, image_size=(1000, 1000), 
    ...
    for frame_number in tqdm.tqdm(range(animation_duration), desc="Animating robots time", disable=not(debug)):  # Number of frames in the animation
        ...
        for i in range(num_robots):
            ...
            # rotate the image of the robot based on the theta
            ax = draw_robot_at_pos_w_rot(robot_image, theta, x, y, ax)
            # robot_image_rotated = cv2.warpAffine(robot_image, cv2.getRotationMatrix2D((robot_image.shape[1] / 2, robot_image.shape[0] / 2), np.degrees(theta), 1), (robot_image.shape[1], robot_image.shape[0]))
            # x -= robot_image_rotated.shape[1] / 2
            # y -= robot_image_rotated.shape[0] / 2
            # ax.imshow(robot_image_rotated, extent=(x, x + robot_image_rotated.shape[1], y, y + robot_image_rotated.shape[0]))
            # ax.imshow(robot_image, extent=(x-robot_image.shape[1] / 2, x + robot_image.shape[1] / 2, y-robot_image.shape[0] / 2, y + robot_image.shape[0] / 2))
        ...
        if log_output_path:
            # frame_number = frame_number + 1
            # print("Logging frame number:", frame_number)
            # add the frame number at the begining of the line
            log_file.write(f"{frame_number:03d} ")
            log_file = log_pos_array(pos_array, log_file)
...
def draw_robot_at_pos_w_rot(robot_image, theta, x, y, ax):
    theta = theta - np.pi / 2  # Adjust angle to match the robot's orientation
    theta = theta % (2 * np.pi)  # Normalize angle to [0, 2π]
    robot_image_rotated = cv2.warpAffine(robot_image, cv2.getRotationMatrix2D((robot_image.shape[1] / 2, robot_image.shape[0] / 2), np.degrees(theta), 1), (robot_image.shape[1], robot_image.shape[0]))
    x -= robot_image_rotated.shape[1] / 2
    y -= robot_image_rotated.shape[0] / 2
...
def log_pos_array(pos_array, log_file):
    """
    Logs the positions of robots to a file.
    pos_array: list of (x, y) tuples representing the robot's positions
    log_file: file object to write the log to
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        if os.path.exists('etc'):
            prefix_path = "etc/validation/media"
            robot_image_path = f"etc/validation/{robot_str}.png"
        else:
            prefix_path = "/home/p27/LARS/LARS/etc/validation/media"
        ...
        N = 70
        experiment_name = f"robot_grid_image_from_number_{robot_str}_N_{N}" # __{date_str}"
        log_file_path = f"{prefix_path}/{experiment_name}_log.txt"
        generate_robot_grid_from_number(robot_image_path=robot_image_path, num_robots=N, robot_width=robot_size, image_size=image_size,
