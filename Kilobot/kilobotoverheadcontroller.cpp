...
//    connect(&timer,SIGNAL(timeout()), this, SLOT(sendBatch()));
    ...
    qDebug() << "Serial Connection: " << serial_conn->enumerate();
}
...
void KilobotOverheadController::sendDataMessagePub(uint8_t *payload, uint8_t type)
{
    //if (sending) {
